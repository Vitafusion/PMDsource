...
# build confusion data frame
confusion_df = function(pmat, type){
  
  pmat_A = as.matrix(pmat[ytrue==type,])
  Asam = rpmd(pmat_A, n = 10000)
  colnames(Asam) = paste0("Pred:",c("A","B","C","D"))
...
marginal_bar = function(pmat, type){
  ...
  colnames(outputdf) = c("count",paste0("Pred:",c("A","B","C","D")))
